...
def get_log_folder_for_month(log_folder_base: pathlib.Path, now: datetime) -> pathlib.Path:
...
def create_new_log(now: datetime, template_file: pathlib.Path, log_folder: pathlib.Path, force: bool=False, dry_run: bool=False):
    ...
    if not log_file_path.exists() or force:
      ...
      if dry_run:
        print(f"[Dry Run] Would have created copy of file '{template_file.absolute()}'")
...
if __name__ == "__main__":
    ...
    if opts.get('--ago', None):
      log_day = datetime.date.today() - datetime.timedelta(days=int(opts.get('--ago', 0)))
